...
symbol_T search_from_scope_out(char *name, int scope)
{
    ...
    if(tmp != NULL)
    {
        ...
        else
        {
            ...
            while(tmpSubList != NULL && (tmpSubList->scope > scope || tmpSubList->active == 0))
            {
...
/*returns the struct with varName == name and same scope(even if active == 0), otherwise null*/
...
symbol_T addElement(char * varName, int category, int scope, int line, int offset, enum scopespace_t space)
{
    ...
    else/*symbol already exists with that name but with different category*/
    {
        ...
        if(tmp->scope < scope)  /*order by decreasing scope, ..2, 1, 0 (global)*/
        {
        ...
        else
        {
            ...
            else if(prev == NULL)
            {
                if(tmp->scope == scope)
                {
                    elem->nextSublist = tmp->nextSublist;
                    tmp->nextSublist = elem;
                }
            }
...
symbol_T add_anonymus_func(int scope, int line, int offset, enum scopespace_t space)
{
    ...
    elem = addElement(buf, 4, scope, line, offset, space);
...
void print_symbol_table()
{
    ...
    while(tmp != NULL)
    {
        printf("Symbol: %s, scope %d, active %d, category %d, in line %d with offset %d in space %d\n", tmp->varName, tmp->scope, tmp->active, tmp->category, tmp->line, tmp->offset, tmp->space);
        tmpSubList = tmp->nextSublist;
        while(tmpSubList != NULL)
        {
            printf("Symbol: %s, scope %d, active %d, category %d, in line %d with offset %d in space %d\n", tmpSubList->varName, tmpSubList->scope, tmpSubList->active, tmpSubList->category, tmpSubList->line, tmpSubList->offset, tmpSubList->space);
            tmpSubList = tmpSubList->nextSublist;
        }
        tmp = tmp->nextSym;
    }
